...
def get_data():
    ...
    SELECT
        ...
        ganho
    FROM
        public.gold_commodities;
    """
    try:
        df = pd.read_sql(query, engine)
        return df
    except ProgrammingError as e:
        st.error(f"Erro ao acessar a tabela 'gold_commodities' no schema '{DB_SCHEMA}': {e}")
...
if df.empty:
    st.write("Não foi possível carregar os dados. Verifique se a tabela 'gold_commodities' existe no schema especificado.")
else:
    # Exibir os dados
